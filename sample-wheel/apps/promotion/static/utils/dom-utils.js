...
const _VALID_TRANSFORM_RE = /^(matrix|translate|rotate|scale|skewX|skewY)\s*\(/;

/**
 * Strip transform attributes with non-function values (e.g. bare "...") from an
 * SVG markup string before it is assigned to innerHTML.  The HTML parser does NOT
 * treat <style> content inside inline SVG as raw text, so CSS comments like
 * "/* <g transform="..."> *\/" can produce real DOM nodes with invalid transforms.
 * This runs as a string operation so the browser never sees the bad attribute.
 */
export function sanitizeSvgTransforms(svgStr) {
  if (!svgStr || typeof svgStr !== 'string') return svgStr;
  return svgStr.replace(/(\s)transform="([^"]*)"/g, (match, space, val) => {
    const trimmed = val.trim();
    if (!trimmed || _VALID_TRANSFORM_RE.test(trimmed)) return match;
    return '';
  });
}
