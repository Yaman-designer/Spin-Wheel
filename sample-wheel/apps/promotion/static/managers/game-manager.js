...
export function syncGameSvgWithRewardInputs(gameSvgHtml) {
  ...
  const sanitized = sanitizeSvgTransforms(gameSvgHtml);
  const tempContainer = document.createElement('div');
  try {
    tempContainer.innerHTML = sanitized;
  } catch (e) {
    console.warn('[syncGameSvgWithRewardInputs] SVG parse failed:', e);
    return gameSvgHtml;
  }
...
export class GameManager {
  ...
  _replaceCurrentGameSvgMarkup(svgMarkup, gameType) {
    ...
    const _safeFinalSvg = sanitizeSvgTransforms(finalSvg);
    if (!_safeFinalSvg || !_safeFinalSvg.trim()) return false;

    try {
      gameInner.innerHTML = _safeFinalSvg;
    } catch (e) {
      console.warn('[_replaceCurrentGameSvgMarkup] SVG mount failed, gameType:', gameType, e);
      return false;
    }

    if (!gameInner.querySelector('svg')) {
      console.warn('[_replaceCurrentGameSvgMarkup] SVG element missing after mount, gameType:', gameType);
    }
    ...
    const _miniGameId = Number(this.theme?.gameID);
    if (_miniGameId === 2 || _miniGameId === 3) {
      const _miniRewards = _collectRewardsFromGameRewardsDom();
      MiniGameEngine.boot(gameInner, _miniGameId, _miniRewards, null);
    }
  ...
  _updateDOMElements() {
    if (!this.gameTypeSelect || typeof document === 'undefined') {
      return;
    }

    if (this.gameTypeSelect.value !== this.currentView) {
      this.gameTypeSelect.value = this.currentView;
    }
  }
