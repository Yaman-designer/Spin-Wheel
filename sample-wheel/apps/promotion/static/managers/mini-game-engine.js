/**
 * mini-game-engine.js
 * Interactive animations for Slot Machine (gameID=2) and Scratch Card (gameID=3).
 * Called after SVG is mounted into .game-svg-container.
 *
 * Usage: MiniGameEngine.boot(containerEl, gameId, rewards, onResult)
 */
const MiniGameEngine = (() => {
  ...
  function bootSlot(container, rewards, onResult) {
    ...
    const spinLabel = [...svg.querySelectorAll('text')].find(t => /SPIN/i.test(t.textContent));
    const reelEls = [...svg.querySelectorAll('text[font-size="42"]')];
    const creditEl = [...svg.querySelectorAll('text')].find(t => t.textContent.trim() === '100');
    const winEl    = [...svg.querySelectorAll('text')].find(t => t.textContent.trim() === '0');
    ...
    function doSpin() {
      ...
      const stopAt = reelEls.length >= 3 ? 3 : reelEls.length;
      const winSym = '🍒';
      ...
      setTimeout(() => {
        if (isWin) { wins++; if (winEl) winEl.textContent = String(wins); }
        spinning = false;
  ...
  function bootScratch(container, rewards, onResult) {
    ...
    const allRects = [...svg.querySelectorAll('rect')];
    const foil = allRects.find(r => (r.getAttribute('fill') || '').includes('scratchPattern'));
    const ctaBtn = allRects.find(r => Number(r.getAttribute('ry')) >= 20 && Number(r.getAttribute('width')) >= 200 && Number(r.getAttribute('y')) > 300);
    const ctaText = [...svg.querySelectorAll('text')].find(t => /SCRATCH/i.test(t.textContent));
    const prizeLabel = [...svg.querySelectorAll('text')].find(t => t.textContent.trim() === '20% OFF');
    ...
    function reveal() {
      ...
      if (foil) { foil.style.transition = 'opacity 0.7s'; foil.style.opacity = '0'; }

      svg.querySelectorAll('ellipse').forEach(e => { e.style.transition = 'opacity 0.4s'; e.style.opacity = '0'; });
    ...
    if (foil) {
      ...
      foil.addEventListener('touchmove', e => { e.preventDefault(); if (++moved > 8) reveal(); }, { passive: false });
    }
  }
  ...
  function boot(containerEl, gameId, rewards, onResult) {
    ...
    if (id === 2) bootSlot(containerEl, rewards, onResult);
    else if (id === 3) bootScratch(containerEl, rewards, onResult);
  }
