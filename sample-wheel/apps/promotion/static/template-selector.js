...
export default class TemplateSelector {
  ...
  getGameInfo(t) {
    ...
    if (!rec) {
      throw new Error(`Template ${t?.id ?? t?.name ?? 'unknown'} has invalid gameID: ${t?.gameID}`);
    }

    const gameType = rec.name;
    const gameId = rec.id;
    const svg = rec.game;
    const category = rec.name;

    return { gameType, gameId, svg, category };
  }
